...
```bash
paperclipai adapter install --payload-json '{"packageName":"@rizquuula/paperclip-omp-adapter","version":"0.6.2"}' --json
```
...
```bash
TARBALL=$(npm pack)
paperclipai adapter install --payload-json "{\"packageName\":\"$PWD/$TARBALL\",\"version\":\"0.6.2\"}" --json
```
